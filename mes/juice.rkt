...
;;TODO: make style engine-specific
(define mes-style-table
  (pretty-print-extend-style-table #f
    '(define-proc if-else set-arr~ set-arr~b set-arr~c set-arr~d set-reg: set-reg:: set-reg:d set-var slot seg)
    '(define      if      send     send      send      send      set!     set!      set!      set!    set! if)))
